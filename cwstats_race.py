...
def main():
    ...
    if not rows:
        print("Geen race-rows gevonden. Mogelijk is de pagina-structuur veranderd. Gebruik placeholder.")
        rows = []

    stats = parse_clan_stats(soup)
    buckets = parse_battles_left_today(soup)

    race_text = format_race_rows(rows) if rows else "Race standings: no data found."
    ...
    write_files(data, race_text, Path(args.public_dir))
