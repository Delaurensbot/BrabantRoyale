...
type PartialDashboardData = Partial<DashboardData> & {
  sections?: Partial<Record<SectionKey, Partial<Section>>>;
};
...
function sanitizeData(payload: PartialDashboardData | null | undefined): DashboardData {
  if (!payload || typeof payload !== "object") return skeletonData;

  const sections: Record<SectionKey, Section> = {
    race: {
      title: payload.sections?.race?.title || skeletonData.sections.race.title,
      text: payload.sections?.race?.text || skeletonData.sections.race.text,
    },
    clan_stats: {
      title: payload.sections?.clan_stats?.title || skeletonData.sections.clan_stats.title,
      text: payload.sections?.clan_stats?.text || skeletonData.sections.clan_stats.text,
    },
    battles_left: {
      title: payload.sections?.battles_left?.title || skeletonData.sections.battles_left.title,
      text: payload.sections?.battles_left?.text || skeletonData.sections.battles_left.text,
    },
  };

  return {
    generated_at_iso: payload.generated_at_iso || skeletonData.generated_at_iso,
    generated_at_epoch_ms: payload.generated_at_epoch_ms || skeletonData.generated_at_epoch_ms,
    update_interval_seconds: payload.update_interval_seconds || skeletonData.update_interval_seconds,
    sections,
    copy_all_text:
      payload.copy_all_text || `${sections.race.text}\n\n${sections.clan_stats.text}\n\n${sections.battles_left.text}`,
  };
}
...
export default function Page() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const payload = (await res.json()) as PartialDashboardData;
